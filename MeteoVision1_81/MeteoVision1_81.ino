...
String ProductName = "MeteoVision V1.81";
    ...
    if (Currentmode == MODO_NOTTE) {
      ...
      if (showText) {
        if (!DisplayOneTime) {
          ...
          // Attiva la sequenza vento quando mostri le informazioni (solo in AUTO e se abilitata)
...
void displayWeatherInfo() {
  ...
  // Aggiorna i LED solo se siamo in modalità AUTO
  if (Currentmode == MODO_AUTO) {
    updateWeatherLEDs();
...
void updateWeatherLEDs() {
  ...
  if (Currentmode != MODO_LED_OFF && Currentmode != MODO_ALL_OFF && Currentmode != MODO_NOTTE) {
    ...
    // LOGICA DI ACCENSIONE LED BASATA SUL CODICE METEO
    // TEMPORALE (200-299)
    if (weatherConditionCode >= 200 && weatherConditionCode < 300) {
    ...
    // PIOGGERLLA (300-399) o CONDIZIONI ATMOSFERICHE (700-799)
    ...
    // PIOGGIA (500-599) o NEVE (600-699)
...
void SetRingColor(int Mode) {
  ...
  if (Mode == MODO_LED_OFF || Mode == MODO_ALL_OFF || Mode == MODO_NOTTE) {
    ...
    setNeopixelColor(neopixelBaseColor);
  } else if (Mode == MODO_AUTO) {
    // Modalità AUTO: LED individuali gestiti dal meteo, anello animato
    animateNeopixelCircular();
  }
...
void checkNightMode() {
  ...
  if (!nightModeEnabled) {
    if (nightModeActive) {
      ...
      SetRingColor(Currentmode); // Ripristina la modalità precedente
